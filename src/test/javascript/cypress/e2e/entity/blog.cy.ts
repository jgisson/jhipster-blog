...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'joliment population du Québec jusqu’à ce que', handle: 'envisager en dedans de' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('de manière à produire');
      cy.get(`[data-cy="name"]`).should('have.value', 'de manière à produire');

      cy.get(`[data-cy="handle"]`).type('terne amorcer');
      cy.get(`[data-cy="handle"]`).should('have.value', 'terne amorcer');
